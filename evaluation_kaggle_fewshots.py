...
emotion_df = pd.read_csv(f'data/cleaned_data/kaggle_emotion_{n_samples}_sample.csv', index_col=0)
sampled_df = emotion_df.sample(3, replace=False)
...
def main():
    ...
    with open(f'fewshots_kaggle_emotion_{n_samples}_predicted_list.pkl', 'wb') as f:
        pickle.dump(predicted_list, f)

    with open(f'fewshots_kaggle_emotion_{n_samples}_label_list.pkl', 'wb') as f:
